...
for i,q in enumerate(questions) :
    ...
    # opinions = nutil.grep_string( nutil.grep_string( nutil.grep_string( nutil.grep_string( su_sets , PARTY_DIC.values() ) ,["代表"] ), ["立場"] ) , [AGREE_SIGHN,NON_AGREE_SIGHN])
    ...
    for op in opinions : 
        ...
        if not bool( gou_iter )  : continue     # 全て賛成/反対のみであれば、次の発言へ
    ...
    for l in q["ProsConsPartyList"] :
        if not l in party_opinion.keys() :
